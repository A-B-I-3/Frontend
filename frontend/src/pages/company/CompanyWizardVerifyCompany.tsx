...
export function CompanyWizardVerifyCompany() {
  const { t } = useTranslation()
  const navigate = useNavigate()
  const { state } = useLocation() as { state?: { email?: string } }
  const { interviewerId, setInterviewerStagingToken } = useOnboarding()
  ...
  async function handleComplete(code: string) {
    ...
    try {
      ...
      client.defaults.headers.common.Authorization = `Bearer ${data.stagingToken}`
      navigate('/company/register/personal')
    } catch (err: any) {
